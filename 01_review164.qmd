...
```{r}
vaccine_data |>
  filter(state %in% c("Iowa", "Wisconsin", "North Dakota", "South Dakota", "Minnesota")) |>
ggplot(aes(x = perc_Biden, y = perc_complete_vac, color = state)) +
  geom_point() +
  geom_smooth(se = FALSE) +
  labs(
    x = "Votes for Biden", 
    y = "Percent of Complete Vaccinations", 
    title = "Votes for Biden by Percent of Complete Vaccinations"
  )
```
...
**I prefer the first plot, so states can be compared to each other in an easier way, since they overlap. **
...
```{r}
vaccine_data |>
  mutate(above = median_HHincome_2019 >= 69560) |>
  group_by(region) |>
  summarize(prop_above = mean(above)) |>
...
**It is treated as a logical vector which means that is it seen as a true or false (0 or 1), which you cant take the mean of, so you have to specify to order regions by the proportion of above median income. **
...
vaccine_data |>
  mutate(
    HHincome_vs_national = ifelse(median_HHincome_2019 < 69560, 
                                       "Below median", 
                                       "Above median")) |>
  mutate(region_sort = 
           fct_reorder(region, 
                       median_HHincome_2019 >= 69560,
                       .fun = mean, 
                       .desc = TRUE
                       )) |>
...
**Different plots are produced because the first plot divides total population by 1000000, and returns a precise number with decimals. The second code returns remainders, so rounds down to one significant figure, so there is a large density at 0 and 1, rather than being continuous decimal values. **
...
```{r}
vaccine_data |>
  ggplot(aes(x = log10(tot_pop))) +
  geom_histogram(bins = 20, fill = "hotpink") 
```

**This is a unimodal distribution and a lot less right skewed than the non-logged population histogram, suggesting that population is more log-distributed rather than normally distributed on a normal scale. **
...
```{r}
vaccine_data |>
  ggplot(aes(x = log10(tot_pop), y = perc_complete_vac, color = metro_status)) +
...
vaccine_data |>
  mutate(
    as.factor(rural_urban_code)
  ) |>
ggplot(aes(x = rural_urban_code, y = perc_complete_vac)) +
  geom_violin(fill = "seagreen", alpha ) 

vaccine_data |>
  mutate(
    as.factor(rural_urban_code)
  ) |>
ggplot(aes(x = rural_urban_code, fill = perc_complete_vac)) +
